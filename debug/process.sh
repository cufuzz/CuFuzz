...
if [ -f "$target_dir/$executable1" ]; then
    run_and_tail "$target_dir/$executable1"
else
...
if [ -f "$target_dir/$executable1" ]; then
    run_and_tail "compute-sanitizer $target_dir/$executable1"
else
...
if [ -f "$target_dir/$executable2" ]; then
    run_and_tail "$target_dir/$executable2"
else
...
if [ -f "$target_dir/$executable2" ]; then
    run_and_tail "compute-sanitizer $target_dir/$executable2"
else
